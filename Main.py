...
#     print(N, len(data))
    for timestep in range(N-1):
        data_timestep = data[timestep]
        measurement_data_timestep = data_timestep[0]
        movement_data_timestep = data_timestep[1]
        
        pos_parameter_offset = 2*timestep 
#         print("Timestep: ", timestep)
        
        # for measurement of landmarks in a timestep
        for value in measurement_data_timestep:
            landmark_number = value[0]
            landmark_index = -2*(num_landmarks - landmark_number)
#             print(landmark_number)
            dist_x = value[1]
            dist_y = value[2]
            # print(landmark_number, dist_x, dist_y)
            # print(xi.value)
            # for x and y
            Ohm.value[pos_parameter_offset + 0][pos_parameter_offset + 0] += 1.0/measurement_noise
            Ohm.value[pos_parameter_offset + 1][pos_parameter_offset + 1] += 1.0/measurement_noise
            # for landmarks x and y
            Ohm.value[landmark_index+0][landmark_index+0] += 1.0/measurement_noise
            Ohm.value[landmark_index+1][landmark_index+1] += 1.0/measurement_noise
            
            # for x between pos and landmark
            Ohm.value[pos_parameter_offset + 0][landmark_index+0] -= 1.0/measurement_noise
            Ohm.value[landmark_index+0][pos_parameter_offset + 0] -= 1.0/measurement_noise
            
            # for y between pos and landmark
            Ohm.value[pos_parameter_offset + 1][landmark_index+1] -= 1.0/measurement_noise
            Ohm.value[landmark_index+1][pos_parameter_offset + 1] -= 1.0/measurement_noise
            
            # for xi
            # for x pos
            # print(dist_x/measurement_noise)
            xi.value[pos_parameter_offset + 0][0] -= dist_x/measurement_noise
            xi.value[landmark_index + 0][0] += dist_x/measurement_noise
            
            # for y pos
            xi.value[pos_parameter_offset + 1][0] -= dist_y/measurement_noise
            xi.value[landmark_index + 1][0] += dist_y/measurement_noise
            
#             print(value)
# #             print(1.0/measurement_noise, Ohm.value[parameter_offset + 0][parameter_offset + 0])
#             Ohm.show()
#             xi.show()
#             input()
        
#         print(timestep, parameter_offset, n_para_eachtimestep)
        
        # for movement in a timestep
        # for x and y of initail point
        Ohm.value[pos_parameter_offset + 0][pos_parameter_offset + 0] += 1.0/motion_noise
        Ohm.value[pos_parameter_offset + 1][pos_parameter_offset + 1] += 1.0/motion_noise
        
        # for x and y of final point
        Ohm.value[pos_parameter_offset + 2 + 0][pos_parameter_offset + 2 + 0] += 1.0/motion_noise
        Ohm.value[pos_parameter_offset + 2 + 1][pos_parameter_offset + 2 + 1] += 1.0/motion_noise
        
        
        # for x between initial point and final point
        Ohm.value[pos_parameter_offset + 0][pos_parameter_offset + 2 + 0] -= 1.0/motion_noise
        Ohm.value[pos_parameter_offset + 2 + 0][pos_parameter_offset + 0] -= 1.0/motion_noise
        
        # for y between pos and landmark
        Ohm.value[pos_parameter_offset + 1][pos_parameter_offset + 2 + 1] -= 1.0/motion_noise
        Ohm.value[pos_parameter_offset + 2 + 1][pos_parameter_offset + 1] -= 1.0/motion_noise
        
        # for xi
        # for x pos
        xi.value[pos_parameter_offset + 0][0] -= movement_data_timestep[0]/motion_noise
        xi.value[pos_parameter_offset + 2 + 0][0] += movement_data_timestep[0]/motion_noise
        
        # for y pos
        xi.value[pos_parameter_offset + 1][0] -= movement_data_timestep[1]/motion_noise
        xi.value[pos_parameter_offset + 2 + 1] [0]+= movement_data_timestep[1]/motion_noise
#         Ohm.show()
#         xi.show()
